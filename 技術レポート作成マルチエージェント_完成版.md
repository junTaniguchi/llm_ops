...
最初に完成形を示し、その後、**PoC、本番導入、本番導入後**の順で、各段階に必要な設計と、実装判断に必要な主要コードを説明する。PoCではマルチエージェントとして成立する最小契約と評価を優先し、本番導入時に文書集合、Prompt、権限、Human-in-the-loop、Release、Monitoringの統制を追加する。本番導入後は、本番Traceと専門家Assessmentを根拠として必要な高度化だけを追加する。

なお、EvaluationDataset、Scorer、Judge、Prompt、Retrievalの一般的な改善ライフサイクル、Production → Development → Staging → Productionの環境間還流、文書集合スナップショットやRAGリリース構成台帳など**RAGエージェントと共通する設計は「社内技術文書検索システム（エージェント型RAG）_完成版.md」を正とする**。本資料では同じ説明を繰り返さず、マルチエージェントで追加になるAgent間契約、State、処理経路、Loop、Human-in-the-loop、複数Promptの一括固定を中心に説明する。
...
| Release構成 | Prompt Version、Model Service、文書集合スナップショット、Git Commit | Quality／Release工程 | Request開始時に不変IDへ固定し、途中で再解決しない |
...
| --- | --- | --- | --- |
| Research Agent | テーマ、Review指摘、文書集合スナップショット、閲覧権限、公開Policy | Evidence、調査メモ | 権限外・掲載不可文書の取得、出典の捏造 |
...
| 機能 | 所有Bundle | 責務 |
| --- | --- | --- |
| 文書Pipeline／AI Search | `knowledge` | 文書解析、Chunk、文書集合スナップショット、Index同期、Policy Filter付き検索を提供する。 |
...
本資料では、MLflow／Databricksの公式Resourceと、技術レポート作成アプリケーション固有の管理Recordを混同しない。特に**Prompt Bundle構成台帳、Release参照、リリース判定、実行時人手確認記録、品質レビュー記録**は、複数の公式機能を組み合わせて運用判断を行うための本資料側のData Model／Workflowであり、単独のDatabricks／MLflow標準Resource名ではない。
...
| 本資料独自のData Model／Workflow | Prompt Bundle構成台帳、Release参照、リリース判定、実行時人手確認記録、品質レビュー記録 | 標準機能だけでは表せない「複数Versionの一括固定」「人の承認」「原因と改善先の追跡」に限定して使う |
| 外部運用システム | Jira、GitHub Issues、ServiceNow、Slack／Teams等 | Incident、作業チケット、通知の正本候補。Databricks側へ同じ状態を二重管理しない |

**意思決定記録（本資料独自用語）**は、PoCのGo／No-Go、本番Release、Rollback、Risk受容、利用範囲拡大、Human Review例外承認など、人間が行った重要な判断を、Evaluation Run ID、Trace ID、Prompt Bundle ID、文書集合スナップショットID、承認者、理由とともに保存する追記型Recordである。実行Logそのものではなく、実行証跡を根拠として「誰が何を判断したか」を残す。
...
| 文書集合スナップショット | ある時点で検索対象となる全文書Version集合。取り込みRun IDとは分ける。物理名は`corpus_snapshot_id`等を維持する |
| Prompt Bundle | Research／Outline／Writer／Review等の不変Prompt Version、Model、文書集合、Gitを一組として固定したRelease構成 |
| Prompt Bundle構成台帳 | Prompt Bundleの不変構成を保存する本資料独自の管理Record。物理実体は`technical_report_prompt_bundles` |
| Release参照 | `production`等が現在どのPrompt Bundleを使うかを指す可変Pointer。物理実体は`technical_report_release_channels` |
...
| リリース判定 | Holdout、Security、Evidence、処理経路、性能、CostからRelease可否の判定材料を作る本資料独自ワークフロー |
...
PoC章では「マルチエージェントとして成立するか」、本番導入章では「再現・統制・停止・復旧できるか」、本番導入後章では「本番証跡からAgent／State／処理経路の問題を切り出せるか」を中心に読む。
...
| --- | --- | --- | --- | --- |
| PoC | 共通Schema、初期Prompt、固定dev Corpus、AI Search、LangGraph、Trace、EvaluationDataset、Scorer、最小UI | Evidence件数、Agent経路、Loop、引用、Trace、限定Testerの結果を確認 | Research／Outline／Writer／Review／処理経路に原因分解し、1要素ずつ比較 | マルチエージェントとして成立するか、本番化GapとGo／No-Goを決める |
| 本番導入／Pilot | 文書集合スナップショット、Prompt Bundle、Server側権限・公開Policy、Durable HITL、Training／Holdout、リリース判定、Production Monitoring、Rollback | ACL、掲載可否、Checkpoint、Agent／Model／Search、Latency、Cost、Human Reviewを監視 | Dry Run／Pilot結果から閾値、手順、Policy、利用Scopeを調整 | Release継続、Rollback、No-Go、利用範囲拡大を責任者が承認 |
...
| 比較軸 | PoC | 本番導入 | 本番導入後 |
| --- | --- | --- | --- |
| 文書集合 | dev用の固定Corpus／Indexを手動指定 | RAG基盤の文書集合スナップショットで全文書Version集合を固定 | 更新履歴を継続し、品質問題時に検証済み前版へRollback |
...
| 評価 | 小規模EvaluationDataset、決定論的Scorer＋必要最小のJudge | Training／Holdout、リリース判定、Production Monitoringを本番前に用意 | 本番AssessmentでDatasetとJudgeを育成 |
...
| PoC版 | 本番版での変更 | 理由 |
| --- | --- | --- |
| 固定文字列／環境変数の`corpus_snapshot_id` | `corpus_snapshot_manifest`／`registry`で不変な文書集合スナップショットを発行 | 文書Version集合を再現・Rollback可能にする |
| dev Prompt URIの固定 | `technical_report_prompt_bundles`＋Release参照 | 複数Prompt、Model、Corpus、GitをAtomicにReleaseする |
| 最小`human_review_node()`で終了 | Durable Checkpoint＋実行時人手確認記録＋`Command(resume=...)` | 外部承認待ちや長時間Requestを安全に再開する |
...
| 文書集合スナップショット | 固定dev値 | ○。RAG資料の共通基盤を利用 | ○ |
| Prompt Bundle構成台帳／Release参照 | －。固定dev値 | ○。マルチエージェント固有 | ○ |
| Durable Checkpoint／実行時人手確認記録 | 最小エスカレーションのみ | ○。外部公開・高Risk等で必要 | ○。SLA／再割当を高度化 |
| Production Monitoring | － | ○。RAG資料の共通基盤へAgent／Route／Loop Signalを追加 | ○。Sampling／Thresholdを調整 |
| 品質レビュー記録 `technical_report_review_cases` | － | 必要に応じ最小利用 | ○。定期Review自動化時に本格利用 |
| Prompt Optimization／Judge Alignment | － | 共通手順はRAG資料参照 | 条件付き高度化 |
| Request単位カナリア／State Replay | － | － | 条件付き高度化 |
...
| 処理経路 | `research`、`reoutline`、`rewrite`、`human_review` | 最終品質だけでは誤った差し戻し先を検出できない |
...
逆に、ResearchとOutlineが正しく、Writerも正しいDraftを作っているのにReviewerが`rewrite`を返した場合は、Reviewerまたは処理経路が改善対象である。
...
| 処理経路 | 期待Route、Loop上限、Human Review条件 |
...
| 実装しない | 文書集合スナップショット台帳、Prompt Bundle構成台帳／Release参照、Durable HITL、Production Monitoring定期実行、Prompt Optimization、Judge Alignment、カナリア |
...
**PoCの非目的**は、本番レベルの職務分離、永続的なレビュー待ちキュー、完全なRelease Workflow、全社Monitoringを完成させることである。これらはPoCの成立性が確認できた後に4章で追加する。
...
**該当なし。** PoCでは文書集合スナップショット台帳、Prompt Bundle構成台帳、Release参照、永続的な実行時人手確認Storeを作らない。EvaluationDatasetとTraceは存在するが、本番の管理系ERをPoCへ持ち込まない。
...
Prompt RegistryはTemplateとVersionを管理する。実際の生成AIモデルはPrompt Bundle構成台帳の`model_service`が指定するFoundation Model APIsのModel ServiceまたはServing Endpointであり、Prompt Registryからモデル推論が実行されるわけではない。`@development`、`@candidate`、`@production` Aliasは品質管理ジョブが候補Versionを選び、後述する不変Version URIのManifestを作るまでにだけ利用する。RuntimeはAliasを解決しない。
...
PoCではRelease参照やDelta Manifestを作らず、評価する不変Prompt URI、Model Service、固定Corpus ID、Git Commitを環境変数から1つの`PromptBundle`へまとめる。本番章で同じ型を`technical_report_prompt_bundles`から読み込む実装へ置換する。
...
def human_review_node(state: ReportState) -> dict[str, Any]:
    # 最小構成では実行時人手確認IDを発行して未承認Draftを返し、Graphを終了する。
...
Databricks Apps Resource
  ↓ PROMPT_BUNDLE_ID
Prompt Bundle構成台帳.model_service
  ↓
...
必須節、禁止語、ID存在、集合包含、Loop、公開区分などはApplication Codeで決定論的に判定する。技術的主張と引用の意味的対応、指摘反映、Reviewerの見逃しだけをLLM Judgeへ任せる。Judge自体は専門家AssessmentでAlignmentし、リリース判定では決定論的Security違反を平均Scoreで相殺しない。
...
この実装では、StreamlitがAgent Serverの`/invocations`へ`stream=true`でRequestを送り、`response.output_text.delta`だけを`st.write_stream()`へ渡す。利用者は対象読者、要件、利用目的を申告するが、`audience`は文体指定であって権限根拠ではない。Reader Entitlement、公開区分、機密上限、文書集合スナップショット、Prompt BundleはServer側で決定する。
...
固定したCorpus、Prompt、Model、Git CommitでEvaluation Runを作り、文書種別、Topicカテゴリ、Publication Scope、期待Route、Human Review要否でSlice比較する。変更する場合はRetrieval、Prompt、Model、処理経路のうち1種類に限定し、新しいEvaluation Runとして比較する。
...
| 処理経路 | Expected Route一致率、不要Loop率 | Review判断とGraph Edgeを分けて確認する |
...
8. Policy、文書集合スナップショット、Prompt Bundleが全Nodeで同じ値のまま維持される。
...
PoC完了時には、KPI、Evaluation Run、代表Trace、失敗原因、未解決Risk、Latency／Token／Loop Cost、本番化Gap、Go／No-Goを簡易な意思決定記録へ残す。
...
| 処理経路 | `research`／`reoutline`／`rewrite`を区別し、Loop上限が効く |
...
本番化Gapには、少なくとも文書集合スナップショット管理、Prompt Bundle Release、認証／公開Policy、Durable HITL、Training／Holdout、Production Monitoring、Rollbackを含める。
...
| --- | --- | --- |
| RAG共通 | 固定dev文書集合 → 文書集合スナップショット | 実装詳細はRAG資料を参照。レポートRequestでは`corpus_snapshot_id`を固定して使う |
| RAG共通 | Golden Set → Training／Holdout | 分離・育成方法はRAG資料を参照。Agent別Expectationだけ本資料で追加する |
| RAG共通 | 手動Trace確認 → Production Monitoring | 登録・Sampling・AlertはRAG資料を参照。Agent／処理経路／Loop Signalを追加する |
| RAG共通 | Go／No-Go → リリース判定＋意思決定記録 | 共通判定へマルチエージェント固有条件を追加する |
| マルチエージェント固有 | 個別Prompt指定 → Prompt Bundle構成台帳＋Release参照 | 複数Agent Prompt、Model、文書集合、Gitを1組で固定する |
| マルチエージェント固有 | Human Reviewで終了 → Durable Checkpoint＋実行時人手確認記録 | 同じStateから安全にResumeする |
| マルチエージェント固有 | 暗黙の差し戻し → State保持／破棄規則＋処理経路Version | 古い中間成果物の誤再利用を防ぐ |

#### 3.8.1 RAG資料と共通する管理資産は再実装しない

文書集合スナップショット、Training／Holdout、Production Monitoring、一般的なリリース判定、Production TraceのDevelopment／Staging還流は、**「社内技術文書検索システム（エージェント型RAG）_完成版.md」**を正とする。

マルチエージェント側では、これらの資産を新しく別方式で作るのではなく、次の参照情報をRequest State／Traceへ追加する。
...
Trace Tagは「そのRequestが何を使ったか」の証跡であり、構成の正本ではない。文書集合スナップショットのMember集合はRAG基盤、Prompt Bundleの構成はPrompt Bundle構成台帳、現在の本番BundleはRelease参照を正本とする。

#### 3.8.2 マルチエージェントで追加する管理資産

| 追加資産 | 物理的な実体 | 必要になる理由 |
| --- | --- | --- |
| Prompt Bundle構成台帳 | `main.llmops.technical_report_prompt_bundles` | Research／Outline／Writer／Review等の不変Prompt Version、Model、文書集合、Gitを評価済みの1組として固定する |
| Release参照 | `technical_report_release_channels` | `production`が参照するBundleを1 Pointerで切り替え、部分更新を防ぐ |
| Durable Checkpoint | LangGraph Checkpointer | Human Review待ちを跨いで同じStateを復元する |
| 実行時人手確認記録 | 人手確認記録Store等 | `thread_id`、Lease、Decision Version、Resume状態を管理する |
| State／Contract Version | Git Commit＋共通Package Version | 古いCheckpointを互換性のない新Graphで再開する事故を防ぐ |

Prompt Bundle構成台帳とRelease参照を分ける理由は、不変の「評価済み構成」と可変の「現在本番が指す構成」を分離するためである。Rollback時は構成台帳を書き換えず、Release参照を前Bundleへ戻す。

Durable Checkpointと実行時人手確認記録も役割が異なる。Checkpointは**Graph Stateそのもの**を保持し、実行時人手確認記録は**誰が、どのDecisionで、どこへResumeするか**を管理する。
## 4. 本番導入時に実施するもの
...
| Rollback | CodeだけでなくPrompt Bundle、モデル経路、文書集合スナップショットを前Releaseへ戻せる |
| Operations | Dry Run、Alert、Runbook、責任者、意思決定記録の入口が用意される |

### 4.2 本番導入時のデータモデル全体像

本番導入では、RAG資料ですでに定義された文書集合スナップショット、AI Search、Training／Holdout、Production Monitoringを共通基盤として利用し、その上にマルチエージェント固有のPrompt Bundle、State、Human-in-the-loop管理を追加する。
...
flowchart TD
    ...
    PB["2. Prompt Bundle構成台帳<br/>Prompt・Model・文書集合・Git"] --> G
    G --> HITL["4. Durable HITL<br/>Checkpoint／実行時人手確認記録"]
    G --> TRACE["5. Trace／Production Monitoring"]
    HOLD["6. Training／Holdout"] --> GATE["7. リリース判定"] --> PB
    REF["8. Release参照"] --> PB
```

初読では、**RAG基盤そのものをこの資料で再構築するわけではない**ことを押さえる。マルチエージェント側の追加点は、Prompt Bundleの一括固定、Stateの永続化、処理経路とLoopの評価である。
...
| 分類 | 物理名／実体 | 本資料からの使い方 |
| --- | --- | --- |
| 文書集合 | `main.llmops.corpus_snapshot_manifest`／`registry` | Request開始時に検証済み`corpus_snapshot_id`を受け取りStateへ固定する |
| 検索 | AI Search Index／Index Release | Research AgentだけがPolicy Filter付きで利用する |
| 評価 | `main.llmops.technical_report_train`／`holdout` | 共通のTraining／Holdout規則を利用し、Agent別Expectationを追加する |
| Monitoring | MLflow Production Monitoring／Registered Scorer | 共通SignalにAgent、処理経路、Loop、HITLを追加する |

**マルチエージェントで追加するもの**

| 分類 | 物理名／実体 | 主な役割 |
| --- | --- | --- |
| Release | `main.llmops.technical_report_prompt_bundles` | 複数Prompt、Model Service、文書集合スナップショット、Git Commitを1 Releaseへ固定 |
| Release | `technical_report_release_channels` | `production`等のCurrent PointerをAtomicに切替 |
| Runtime | Durable Checkpoint Store | `thread_id`単位でGraph Stateを永続化 |
| Runtime | 実行時人手確認記録 Store | Lease、Decision Version、再開状態を保持。物理製品はPlatform標準に従う |
...
`CORPUS_SNAPSHOT_REGISTRY`はRAG基盤側の正本を参照するだけで、本資料で別Tableを作らない。実行時人手確認記録／Checkpointの物理Schemaは組織のPlatform標準へ依存するため、ここでは論理関係だけを示す。品質改善用の`technical_report_review_cases`は完了済みTraceを扱う別目的のRecordであり、このRuntime ERへ混ぜない。
...
flowchart TB
    ...
    subgraph Realtime["realtime Bundle: 技術レポート作成"]
        ...
        U["利用者"] --> UI["Databricks Apps / Streamlit"]
        UI --> AS["MLflow Agent Server @stream"]
        PB["Prompt Bundle構成台帳／Release参照"] --> AS
        AS --> RA["Research Agent"]
        I -->|検索Evidence| RA
        RA --> OA["Outline Agent"]
        OA --> WA["Writer Agent"]
...
文書集合スナップショットの発行、AI Search公開、Training／Holdout、Production Monitoringの共通実装はRAG資料を参照する。本資料では、Request開始時にその不変IDを固定し、複数Agentの処理途中で再解決しないことを追加要件とする。
...
| 4 | 4.3.4 | Prompt Bundle、HITL、処理経路評価等の実装 | 冒頭の実装ブロック表を読み、必要な節だけ詳細へ進む |

#### 4.3.1 PoCコードからの主な変更点

| PoC | 本番Baseline | 変更理由 |
| --- | --- | --- |
| 固定dev Corpus ID | RAG基盤の文書集合スナップショット | 全文書Version集合を再現する |
| 個別Prompt Version指定 | Prompt Bundle構成台帳 | 複数Agent PromptをAtomicにReleaseする |
| 固定Tester | 認証済みUser／Group＋Server Policy | Client入力による権限昇格を防ぐ |
| Human Reviewで終了 | Durable Checkpoint＋実行時人手確認記録＋Resume | 長時間承認待ちから安全に再開する |
...
3. RAG基盤の文書集合スナップショットを参照する
        ↓
4. Prompt BundleでPrompt・Model・文書集合・Gitを固定する
        ↓
5. Identity／Publication PolicyをServer側で解決する
        ↓
6. Durable HITLで停止・再開を永続化する
        ↓
7. Training／Holdoutとリリース判定を用意する
        ↓
...
| `knowledge` | 文書解析、文書集合スナップショット、AI Search Index |
| `quality` | Prompt、Training／Holdout、リリース判定、Monitoring |
...
| Evaluation Experiment | Training／Holdout、Candidate比較、リリース判定 |
...
| --- | --- | --- | --- |
| RAG共通基盤の参照 | 4.3.4.1、4.3.4.5 | 文書集合スナップショット、Training／Holdout | 実装詳細はRAG資料を参照 |
| Prompt Bundle／Release | 4.3.4.2、4.3.4.7 | 複数Prompt、Model、文書集合、GitをRequest単位で固定 | **マルチエージェント固有。重点的に読む** |
| Identity／公開Policy | 4.3.4.3 | 閲覧権限と成果物掲載可否を分離 | 共通ACLにレポート公開Policyを追加 |
| Durable HITL | 4.3.4.4 | 停止、Checkpoint、Decision、Resume | **マルチエージェント固有。重点的に読む** |
| リリース判定／Monitoring | 4.3.4.6、4.3.4.8 | 共通品質GateへAgent／処理経路／Loop条件を追加 | 差分だけ説明 |
| Version追跡 | 4.3.4.9 | Graph／State ContractをGitとTraceへ関連付け | 差分だけ説明 |
| 長文・統制 | 4.3.4.10〜4.3.4.11 | Context Budget、節単位実行、金融向け統制 | マルチエージェント特有の運用論点 |

ここから個別Sourceへ進む。共通RAG実装を読み直す必要がある場合は、**「社内技術文書検索システム（エージェント型RAG）_完成版.md」**へ戻る。
##### 4.3.4.1 RAG共通の文書集合スナップショットを利用する

文書集合スナップショットの発行、Manifest／Registry、AI Searchへの同期、Rollback方法は、**「社内技術文書検索システム（エージェント型RAG）_完成版.md」**の実装をそのまま利用する。本資料では同じ`publish_corpus_snapshot.py`を再掲しない。

マルチエージェント側で追加する契約は次の3点だけである。

1. Request開始時に、検証済み`corpus_snapshot_id`をPrompt Bundle構成台帳から取得する。
2. `ReportState.corpus_snapshot_id`へ固定し、Researchを含む後続NodeでCurrent Pointerを再解決しない。
3. Traceへ`corpus_snapshot_id`を残し、Evidenceの`document_version`と同じ文書集合スナップショットに属することを検証する。
...
##### 4.3.4.2 Prompt Bundle構成台帳をRuntimeで解決する

この実装では、`PROMPT_BUNDLE_ID`からDelta表のManifestを1回取得し、5つの不変Prompt Version URI、Model Service、文書集合スナップショット、Git Commitを解決する。Manifestは`quality` Bundleが発行し、`realtime` Bundleは読み取り専用で利用する。1 RequestのStateへ解決結果をコピーするため、処理中のRelease切替やAlias変更でPromptが混在しない。
...
Production切替は、品質管理ジョブがHoldoutとリリース判定を通過した候補Manifestを新規Insertし、Release参照の`production` Pointerを1 Transactionで新Bundle IDへ切り替える。Databricks Appへは解決済みの`PROMPT_BUNDLE_ID`を設定するか、Request開始時にRelease参照を解決する。Rollbackは前Bundle IDへPointerを戻すだけであり、Manifest行を上書きしない。Online Experimentでは`request_id`または`session_id`の安定HashでBundle IDを一度選び、StateとTraceへ固定する。`@production` AliasはManifest作成時の候補選択にだけ利用し、Runtimeでは利用しない。

Databricks SQL Statement Executionは標準機能だが、AtomicなRelease参照更新、承認Workflow、Manifest不変性の制約は本資料で設計する運用実装である。
...
上の`human_review_node()`は最小構成である。`review_case_id`を付けた未承認成果物を返し、自動公開せず、Graphを終了する。Agentは`review_case_id`をTrace Tagへ残し、品質管理ジョブが同じIDでReview Queueへ冪等登録する。人間の判断後に同じGraphを再開する機能ではない。
...
| 項目 | 本番実装の責務 |
| --- | --- |
| 関連付け | `request_id`、`review_case_id`、LangGraph `thread_id`、Trace IDを実行時人手確認記録へ保存する。 |
| 二重処理 | `review_case_id`とDecision Versionに一意制約を設け、Lease所有者だけが状態を遷移させる。 |
| 冪等性 | 実行時人手確認記録の登録、外部通知、公開処理へIdempotency Keyを渡し、`interrupt()`前の副作用を再実行可能にする。 |
...
##### 4.3.4.5 TrainingとHoldoutへマルチエージェントExpectationを追加する

Training／Holdoutの分離方針、EvaluationDatasetの育成方法、Production TraceからのCase追加は、**「社内技術文書検索システム（エージェント型RAG）_完成版.md」**を参照する。本資料では分割ロジックを再実装しない。
...
同じTopicの言い換えや同じ失敗原因グループをTrainingとHoldoutへ跨がせない点もRAG資料と同じである。
##### 4.3.4.6 共通のリリース判定へマルチエージェント固有条件を追加する

ACL、Groundedness、Citation、Latency、Cost等の共通判定はRAG資料のリリース判定を利用する。本システムでは、そこへ次の条件を追加する。
...
| Bundle | 1 Request内でPrompt／Model／文書集合Versionが変化 |

品質管理ジョブは共通判定とこれらのマルチエージェントSignalをまとめて**判定材料**を作る。最終的な採用／却下／Risk受容は人が決定し、意思決定記録へ残す。
##### 4.3.4.7 Prompt Bundle構成台帳をReleaseする

マルチエージェントでは5つのPromptが組み合わさるため、個別Aliasだけでは「本番でどの組合せだったか」を表しにくい。Release時に各Promptの不変Version URI、Model Service、文書集合スナップショット、Git Commitを`main.llmops.technical_report_prompt_bundles`へ1行のManifestとして固定する。
...
- 文書集合スナップショット、Prompt Bundle、Git Commit、Model Service、Policy Version、Review承認を不変IDで監査証跡へ関連付ける。
- Trace、Checkpoint、実行時人手確認記録、EvaluationDatasetへ個人情報・顧客情報の分類、Masking、保持、Legal Hold、削除Policyを個別に適用する。
- Foundation Model APIs、Reranker、Judgeを含む各Designated Serviceについて、データ所在地、越境、再学習、保持、委託先Riskの利用承認を取得する。
- Emergency停止、前Bundle／文書集合へのRollback、未処理の実行時人手確認記録の再割当を定期的に演習する。

- `corpus_snapshot_id`は取り込みRunではなく全文書Version集合とし、Manifestで文書IDとVersionを固定する。
- 文書ID、Version、文書集合スナップショット、Chunk、引用IDをStateへ保持する。
...
- DAB、共通Wheel、Prompt Bundle構成台帳、文書集合スナップショットをRelease単位として記録する。
- Prompt Aliasを複数個順番に切り替えるだけの非Atomic Releaseを避ける。
- RollbackではGit Commitだけでなく、Prompt Bundle、Model Variant、文書集合スナップショットも戻す。
- 文書集合スナップショット更新だけの場合もGolden Topicで期待Evidenceを検索できることを確認する。
...
| Rollback | 前BundleへChannelを戻す | 前Code／Prompt／Model／Corpus組合せで再実行できる | Release／意思決定記録 |
| 処理経路 | 代表的な`research`／`reoutline`／`rewrite` Case | 期待Edgeへ進み不要Stateを破棄する | Route History |
| Loop上限 | Reviewerが継続差し戻しするFixture | 上限到達でHuman ReviewへFail Closed | Loop Count、実行時人手確認記録 |
...
Pilotでは、Production Monitoringを「後から設定する」のではなく開始時点から有効化する。文書集合スナップショット、Prompt Bundle、実モデル経路、Git Commit、Publication Policy Version、Agent経路、Human Review、Latency／Token／Costを同じTraceへ関連付ける。
...
RollbackではGit Commitだけを戻さない。**Prompt Bundle、モデル経路、文書集合スナップショット、Policy参照を含む評価済みRelease構成**へ戻す。判断理由と証跡IDは意思決定記録へ残す。
...
本番導入後も4章のBaselineを作り直さない。文書集合スナップショット、Training／Holdout、Production Monitoring等のRAG共通資産はそのまま継続し、マルチエージェントでは**実行時人手確認記録、品質レビュー記録、State／処理経路の再現資産**だけを追加・高度化する。

#### 5.1.1 まず理解する構成

```mermaid
flowchart TD
    BASE["1. 本番Baseline<br/>RAG共通基盤＋Prompt Bundle＋Graph＋HITL"] --> MON["2. Trace／Production Monitoring"]
    MON --> REV["3. 品質レビュー記録／Assessment"]
    REV --> CAUSE["4. 最初に壊れたAgent／State／処理経路"]
    CAUSE --> FIX["5. 固定State Fixture／State Replay"]
    FIX --> STG["6. StagingでGraph全体確認"]
    STG --> GATE["7. Holdout／リリース判定"]
    GATE --> CAN["8. Request単位カナリア／再Release"] --> BASE
```
...
| 分類 | 物理名／実体 | 継続する役割 |
| --- | --- | --- |
| RAG共通 | `corpus_snapshot_manifest`／`registry`、AI Search、Training／Holdout、Production Monitoring | 文書集合、検索、共通評価・監視。実装はRAG資料を正とする |
| Release | `technical_report_prompt_bundles` | Agent Prompt、Model、文書集合、Gitを不変Bundleへ固定 |
...
| Runtime | 実行時人手確認記録 Store | 人手判断待ち、Lease、Decision、Resume状態 |
| Trace | MLflow Experiment／UC Trace | Agent Span、Evidence、処理経路、Token、結果 |

**本番導入後に追加・高度化するもの**

| 分類 | 物理名／実体 | 役割 | 導入条件 |
| --- | --- | --- | --- |
| Quality Review | `technical_report_review_cases` | Trace、Expectation、原因Agent、改善先、Split、承認、配送状態 | Review件数と担当者が増えた場合 |
| State Fixture | Masking済みState／Evidence／Draft Test Fixture | Agent単体Regression、State破棄規則の再現 | 原因Agentを局所検証したい場合 |
| State Replay | Graph／Policy Version＋保存State | 処理経路、Loop、Resume先の再現 | Routing／HITL Failureが確認された場合 |
...
RuntimeとQualityは目的が違うため、同じCase Tableへ統合しない。
...
実行時人手確認記録は**今止まっているRequestを再開するための運用State**、品質レビュー記録は**完了済みTraceを改善へ配送するための品質Record**である。State Fixture／Replayは後者にだけ紐付ける。
...
flowchart TD
    ...
    M --> Q["品質レビュー候補"]
    Q --> R["専門家Review<br/>原因Agent／期待Route／Expectation"]
    R --> F["固定State Fixture"]
    F --> D["Development<br/>対象Agent／State／処理経路を局所検証"]
    D --> S["Staging<br/>Graph全体・HITL・Loopを確認"]
    S --> G["共通Holdout＋マルチエージェント判定"]
    G -->|合格| C["Request単位カナリア／Release"]
    G -->|不合格| D
```

Production TraceからEvaluationDatasetへ追加する一般的な手順はRAG資料を参照する。本資料では、Review済みCaseに`root_cause_agent`、`expected_routes`、State Fixture等を追加して再現可能にする。
...
本資料で追加するのは、Review時に**Agent／処理経路／Stateのどこで最初に破綻したか**を残し、その時点のStateを再現可能なFixtureへ変えることである。
...
| 種類 | 目的 | 必要なState |
| --- | --- | --- |
| 実行時人手確認 | 今止まっているRequestを安全に再開する | `thread_id`、Checkpoint、Decision Version、現在State |
| 品質レビュー | 完了済みTraceを将来の改善へ使う | Trace ID、Root Cause、期待値、改善対象 |

実行時人手確認のDecisionをそのままEvaluationDatasetの正解へ自動採用しない。品質レビューでは専門家が改めて期待Evidence、期待Route、最初の破綻箇所を確認する。
...
| `improvement_target` | Agent Prompt／処理経路／State／HITL等の改善先 |
...
| 処理経路 | ReviewDecision、Loop Count、State | LangGraph全体 |
...
#### 5.2.5 State Replayで処理経路とLoopを再現する

処理経路の問題は最終レポートだけを保存しても再現できない。問題発生直前のStateをMaskingしてFixture化し、同じReviewDecisionからどのEdgeへ進むかを再実行できるようにする。
...
マルチエージェントのカナリアでは`request_id`または`session_id`を安定Hashし、Prompt、Model、Corpus、Graph Versionを含むBundleをRequest開始時に一度だけ選択する。
...
| Cost／Performance | Loop増大、P95超過 | Scope／Budget確認 | Context、Model、処理経路の最適化 |
...
End Userの👍／👎だけでこれらを判定できないため、RouteやStateを読めるマルチエージェント／LLMOps担当者が技術原因を確定する。
...
一度にResearch PromptとReviewer Promptと処理経路を同時変更すると、改善理由を追えなくなる。まず局所Fixtureで1要素を改善し、その後にEnd-to-End Graphで副作用を確認する。
...
- Topic文字列だけでなくReview指摘、Audience、要件、Reader Entitlement、Publication Scope、文書集合スナップショットを検索条件へ反映する。
...
#### 5.8.4 Review、処理経路、Human-in-the-loop
...
- 実行時人手確認記録にはLease／Decision Version／冪等Resumeを持たせる。
...
Assessment／Judge Alignmentの一般的な運用はRAG資料を参照する。マルチエージェントでは、Application全体の良否だけでなく、**最初に破綻したAgent、Review Action、実Route、State破棄、HITL Resume**を診断項目へ追加する。
...
- DAB、共通Wheel、Prompt Bundle構成台帳、文書集合スナップショット、Policy VersionをRelease証跡へ関連付ける。
- Prompt Aliasを複数個順番に切り替えるだけの非Atomic Releaseを避ける。
- RollbackではGit CommitだけでなくPrompt Bundle、Model Variant、文書集合スナップショットを戻す。
- 文書集合スナップショットだけを更新するReleaseでもGolden Topicで期待Evidenceを検索できることを確認する。
...
本資料のサンプルコードは、文書集合スナップショット、Server側公開Policy、Prompt Bundle、`reoutline`を含むLangGraph、最小エスカレーション、SSE、評価・改善Loopまでを一貫して示す。一方、金融機関ごとの認証Adapter、Checkpoint Store製品選定、人手確認記録のLease API、SIEM／DLP等はPlatform標準へ依存するためInterfaceと責務までを示す。
...
- [ ] Prompt Bundle構成台帳がAgent Prompt、Model、Corpus、Gitを不変Versionで固定する。
- [ ] Request開始後にAliasやChannelを再解決しない。
- [ ] HoldoutとSecurity／Latency／Cost／HITLからRelease判断する。
- [ ] 意思決定記録に採否理由と証跡IDを残す。
- [ ] RollbackでCodeだけでなくPrompt Bundle、Model、文書集合スナップショットを前構成へ戻す。
...
| 文書集合スナップショット | 全文書Version集合を固定した検索対象版。詳細はRAG資料参照 | `corpus_snapshot_manifest`／`registry` | 本番導入 |
| Prompt Registry | Agent PromptをVersion管理するMLflow Resource | UC-backed Prompt | PoC |
| Prompt Bundle構成台帳 | 複数Prompt、Model、文書集合、Gitを一体固定した本資料独自Record | `technical_report_prompt_bundles` | 本番導入 |
| Release参照 | Production等が現在参照するBundle Pointer | `technical_report_release_channels` | 本番導入 |
| LangGraph | Node、State、差し戻しEdgeを制御するGraph | `report_graph.py` | PoC |
| Human-in-the-loop | 人の判断へ停止・再開する実行方式 | Checkpointer、`thread_id`、`interrupt()`／`Command(resume=...)` | 本番導入 |
| 実行時人手確認記録 | 実行中RequestのHuman Review停止・再開状態 | 人手確認記録Store等＋Checkpoint参照 | 本番導入 |
...
| Holdout Dataset | 最終リリース判定専用の未使用Case集合 | `main.llmops.technical_report_holdout` | 本番導入〜 |
...
| 品質レビュー記録 | 完了済みTraceの期待値、原因Agent、改善先、承認、配送を追跡する本資料独自Record | `technical_report_review_cases` | 本番導入後 |
| State Fixture | 特定Agent／処理経路を再現するために固定・MaskingしたState | Test Fixture／Evaluation Artifact | 本番導入後 |
| State Replay | 保存StateとGraph／Policy Versionから処理経路・Loopを再実行する検証 | Replay Test／MLflow Run | 本番導入後 |
| Prompt Optimization | Training CaseからPrompt候補を探索する改善処理。一般手順はRAG資料参照 | Optimization Run＋Prompt Version候補 | 本番導入後 |
| Judge Alignment | 人間FeedbackへJudge定義を合わせる処理。一般手順はRAG資料参照 | Alignment Run＋候補Judge | 本番導入後 |
| Request単位カナリア | 候補Bundle／ModelをRequest単位で固定して限定本番確認する方式 | Assignment＋Trace Variant | 本番導入後 |
| 意思決定記録 | Release／Rollback／Go-No-Go等の人の意思決定を証跡IDと共に残す追記Record | Delta Table／外部Change Record等 | PoC〜 |
| Route History | Research／Reoutline／Rewrite／Human Review等の実処理経路履歴 | `ReportState.route_history`＋MLflow Trace | PoC〜 |
...
- 本番導入時に文書集合スナップショット、Prompt Bundle、Training／Holdout、HITL、Monitoringの全体像がある。
...
- RAG／LLMOps共通の改善ライフサイクルはRAG資料へ参照を寄せ、第2章ではState破棄、Agent間契約、Request単位Bundle固定などマルチエージェント固有の変更順序だけを説明している。
